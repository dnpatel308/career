...
public class CreateAndHandleDeviceReservationUsingCreateSerialEndPoint {
...
//	        	imei1DeviceFieldDTO.setType("imei");
			...
			public void validateResponse(Response response) {
				...
				try {
					id = JDBCComponent.instance()
							.executeQuery("SELECT `id` FROM `serial_number` ORDER BY `id` DESC LIMIT 1", 30, 1000);
				} catch (Exception e) {
					e.printStackTrace();
				}
				String latestSerialNoId = null;
	...
	@GenerateTestCase(includeOnlyInParallel = true, order = 4, configFileName = "DeviceReservation.properties")
			...
			public void validateResponse(Response response) {
				...
				try {
					JDBCComponent.instance().executeQuery("SELECT * FROM `serial_number_reservation` WHERE `id` = '"
							+ response.jsonPath().getInt("id") + "' AND `status` = 'RESERVED'", 30, 1000);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
