...
import com.brodos.device.reservation.test.dto.SerialNumberActionDTO;
 ...
 * 
 */
//@TestCasesGenerator(configFileName = "SerialAction.properties")
@ParallelTestCasesGenerator(numOfThreads = 1)
public class ArchiveUnArchiveSerial {
...
//	        	imei1DeviceFieldDTO.setType("imei");
	...
	@GenerateTestCase(includeOnlyInParallel = true, order = 3, configFileName="CreateSerialNumberEntry.properties")
	public void testCreateSerialNumber(TestCaseData testCaseData) throws Exception {
		...
		TestCaseResponseValidator testCaseResponseValidator = new TestCaseResponseValidator() {
			...
			public void validateResponse(Response response) {
				...
				testCaseData.getMetadata().put("serialNoId", latestSerialNoID);
			  }
			};
			...
			testCaseData.setDependencyOfNextTestCaseData(true);
			testCaseData.setDependentOnTestCaseData(testCaseData);
			testCaseData.getMetadata().put("serialNo", imei);
			testCaseData.getMetadata().put("articleNo", articleNumber);
	}
	
	@GenerateTestCase(includeOnlyInParallel = true, order = 4)
	public void testArchiveSerialNumber(TestCaseData testCaseData) throws Exception {
		
		TestCaseData dependentOnTestCaseData = testCaseData.getDependentOnTestCaseData();
				
		testCaseData.setUrl(String.format(testCaseData.getUrl(), dependentOnTestCaseData.getMetadata().get("serialNoId")));

		SerialNumberActionDTO serialNumberActionDTO = new SerialNumberActionDTO();

		serialNumberActionDTO.setType("archive");

		DeviceReservationActionArgumentsDTO deviceReservationActionArgumentsDTO = new DeviceReservationActionArgumentsDTO();

		serialNumberActionDTO.setArguments(deviceReservationActionArgumentsDTO);

		testCaseData.setStatusCode(202);
		testCaseData.setBody(writeValueAsString(serialNumberActionDTO, JsonInclude.Include.NON_NULL));
		...
		TestCaseResponseValidator testCaseResponseValidator = new TestCaseResponseValidator() {
			...
			public void validateResponse(Response response) {
				...
				List<JSONObject> id = null;
				try {
					id = JDBCComponent.instance()
							.executeQuery("SELECT `reservable`, `is_archived` FROM `serial_number` WHERE `id` = '"
									+ dependentOnTestCaseData.getMetadata().get("serialNoId") + "'", 30, 1000);
				} catch (Exception e) {
					e.printStackTrace();
				}

				Boolean reservable = null;
				Boolean archive = null;

				if (!id.isEmpty()) {
					reservable = id.get(0).getBoolean("1");
					archive = id.get(0).getBoolean("2");
				}

				Assert.assertTrue(response.getStatusCode() == 202);
				LinkedHashMap serial = response.jsonPath().getJsonObject("serial");
				Assert.assertEquals(serial.get("is-reservable"), reservable);
				Assert.assertEquals(serial.get("is-archived"), archive);
			}
		};
		testCaseData.setTestCaseResponseValidator(testCaseResponseValidator);
		...
		testCaseData.setDependencyOfNextTestCaseData(true);
		testCaseData.setDependentOnTestCaseData(testCaseData);
		testCaseData.getMetadata().put("serialNoId", dependentOnTestCaseData.getMetadata().get("serialNoId"));
		testCaseData.getMetadata().put("serialNo", dependentOnTestCaseData.getMetadata().get("serialNo"));
		testCaseData.getMetadata().put("articleNo", dependentOnTestCaseData.getMetadata().get("articleNo"));
	}

	@GenerateTestCase(includeOnlyInParallel = true, order = 5)
	public void testUnArchiveSerialNumber(TestCaseData testCaseData) throws Exception {
		...
		serialNumberActionDTO.setType("unarchive");
		...
		TestCaseResponseValidator testCaseResponseValidator = new TestCaseResponseValidator() {
			...
			public void validateResponse(Response response) {
				...
				Assert.assertEquals(serial.get("is-reservable"), reservable);
				Assert.assertEquals(serial.get("is-archived"), archive);
				String articleNo = (String) dependentOnTestCaseData.getMetadata().get("articleNo");
				String imei = (String) dependentOnTestCaseData.getMetadata().get("serialNo");
				
				try {
					Utils.publishVoucherRelocationEventUsingIMEI(articleNo, imei);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
