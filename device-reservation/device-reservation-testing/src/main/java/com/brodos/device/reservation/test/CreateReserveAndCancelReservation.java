...
public class CreateReserveAndCancelReservation {
...
//	        	imei1DeviceFieldDTO.setType("imei");
	...
	public void testCancelDeviceReservationSuccess(TestCaseData testCaseData) throws Exception {
		TestCaseData dependentOnTestCaseData = testCaseData.getDependentOnTestCaseData();
		String articleNumber = (String) dependentOnTestCaseData.getMetadata().get("articleNo");
		Long eventId = (Long) dependentOnTestCaseData.getMetadata().get("eventID");
		testCaseData.setUrl(
		...
		TestCaseResponseValidator testCaseResponseValidator = new TestCaseResponseValidator() {
			...
			public void validateResponse(Response response) {
				...
				try {
					com.brodos.device.reservation.Utils.publishVoucherRelocationEvent(articleNumber, eventId);
				} catch (Exception e) {
