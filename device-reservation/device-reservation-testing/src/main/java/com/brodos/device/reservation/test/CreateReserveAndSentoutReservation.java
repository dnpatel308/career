...
public class CreateReserveAndSentoutReservation {        
...
//	        	imei1DeviceFieldDTO.setType("imei");
    ...
    public void testDeviceReservationSuccess(TestCaseData testCaseData) throws Exception {
        TestCaseData dependentOnTestCaseData = testCaseData.getDependentOnTestCaseData();
        List<JSONObject> serialNumberReservationDbResult = JDBCComponent.instance().executeQuery("SELECT snr.`id`, sn.number FROM `serial_number_reservation` snr, `serial_number` sn WHERE snr.`serial_number_id` = sn.id AND snr.`id` = " + dependentOnTestCaseData.getMetadata().get("reservationId") + " AND `status` = 'RESERVED'", 30, 1000);
        testCaseData.setUrl(String.format(testCaseData.getUrl(), serialNumberReservationDbResult.get(0).getInt("1")));
        ...
        TestCaseResponseValidator testCaseResponseValidator = new TestCaseResponseValidator() {
            ...
            public void validateResponse(Response response) {
                ...
                sentoutAndValidate(testCaseData, serialNumberReservationDbResult);
            }
        };
    ...
    private void sentoutAndValidate(TestCaseData testCaseData, List<JSONObject> serialNumberReservationDbResult){
        try {
        	String articleNo = (String) testCaseData.getDependentOnTestCaseData().getMetadata().get("articleNo");
            com.brodos.device.reservation.Utils.publishVoucherSentoutEvent(articleNo, serialNumberReservationDbResult.get(0).getString("2"));
            serialNumberReservationDbResult = JDBCComponent.instance().executeQuery("SELECT snr.`id`, sn.number FROM `serial_number_reservation` snr, `serial_number` sn WHERE snr.`serial_number_id` = sn.id AND snr.`id` = " + testCaseData.getDependentOnTestCaseData().getMetadata().get("reservationId") + " AND `status` = 'SENTOUT'", 30, 1000);
