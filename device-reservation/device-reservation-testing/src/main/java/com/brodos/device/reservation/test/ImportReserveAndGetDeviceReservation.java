...
public class ImportReserveAndGetDeviceReservation {
...
//        	imei1DeviceFieldDTO.setType("imei");
	...
	public void testDeviceReservationSuccess(TestCaseData testCaseData) throws Exception {
		...
		Long eventId = Utils.publishVoucherImportEvent(articleNumber);
		...
		testCaseData.setBody(writeValueAsString(deviceReservationRequestDTO, JsonInclude.Include.NON_NULL));
		...
		testCaseData.getMetadata().put("eventID", eventId);
		testCaseData.setDependencyOfNextTestCaseData(true);
	...
	public void testDeviceConfigurationAvailability(TestCaseData testCaseData) throws Exception {
		...
		Long eventId = (Long) dependentOnTestCaseData.getMetadata().get("eventID");
	...
	public void testGetReservationById(TestCaseData testCaseData) throws Exception {
		TestCaseData dependentOnTestCaseData = testCaseData.getDependentOnTestCaseData();
		List<JSONObject> serialNumberReservationDbResult = JDBCComponent.instance()
				.executeQuery("SELECT `id` FROM `serial_number_reservation` WHERE `id` = "
						+ dependentOnTestCaseData.getMetadata().get("reservationId") + " AND `status` = 'RESERVED'", 30,
						1000);
