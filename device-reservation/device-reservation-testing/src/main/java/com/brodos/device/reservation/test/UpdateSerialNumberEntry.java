 ...
 * 
 */
//@TestCasesGenerator(configFileName = "UpdateSerialNumber.properties")
@ParallelTestCasesGenerator(numOfThreads = 1)
public class UpdateSerialNumberEntry {
...
//	        	imei1DeviceFieldDTO.setType("imei");
	...
	public void testUpdateSerialNumber(TestCaseData testCaseData) throws Exception {

		TestCaseData dependentOnTestCaseData = testCaseData.getDependentOnTestCaseData();
		
		List<JSONObject> randomSerialNoId = JDBCComponent.instance().executeQuery(
				"SELECT `id`, `number`, `article_number`, `ean`, `warehouse_no` FROM `serial_number` WHERE `id`='" + dependentOnTestCaseData.getMetadata().get("serialNoId") + "'" ,
				30, 1000);

		String tempSerialNo = null;
		String tempWarehouseNo = null;
		String tempArticleNo = null;
		String tempEAN = null;

		if (!randomSerialNoId.isEmpty()) {
			testCaseData.setUrl(String.format(testCaseData.getUrl(), randomSerialNoId.get(0).get("1")));
			tempSerialNo = randomSerialNoId.get(0).get("2").toString();
			tempArticleNo = randomSerialNoId.get(0).getString("3").toString();
			tempEAN = randomSerialNoId.get(0).getString("4").toString();
			tempWarehouseNo = randomSerialNoId.get(0).get("5").toString();
		}

		ProductCodeDTO productCodeDTO = new ProductCodeDTO();
		productCodeDTO.setType("EAN");
		productCodeDTO.setValue(tempEAN);

		SerialDTO serialDTO = new SerialDTO();

		serialDTO.setProductCodeDTOs(new ArrayList<>());
		serialDTO.getProductCodeDTOs().add(productCodeDTO);
		serialDTO.setArticleNumber(tempArticleNo);
		serialDTO.setNumber(tempSerialNo);

		if (tempWarehouseNo.equals("40")) {
			serialDTO.setWarehouseId(2);
		} else if (tempWarehouseNo.equals("2")) {
			serialDTO.setWarehouseId(40);
		}
		serialDTO.setTenantId((long) 1);
			...
			public void validateResponse(Response response) {
				...
				try {
					id = JDBCComponent.instance()
							.executeQuery("SELECT `warehouse_no` FROM `serial_number` WHERE `id` = '"
									+ randomSerialNoId.get(0).get("1") + "'", 30, 1000);
				} catch (Exception e) {
					e.printStackTrace();
				}
				...
				if (!id.isEmpty()) {
					warehouseNo = id.get(0).get("1").toString();
					warehouseNumber = Integer.parseInt(warehouseNo);
				}

				Assert.assertTrue(response.getStatusCode() == 200);
				LinkedHashMap serial = response.jsonPath().getJsonObject("serial");
				Assert.assertEquals(serial.get("warehouse-id"), warehouseNumber);
